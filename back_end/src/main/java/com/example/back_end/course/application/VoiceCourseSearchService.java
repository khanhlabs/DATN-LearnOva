...
public class VoiceCourseSearchService {
    ...
    private boolean matches(PublicCourseResponse course, SearchFilters filters) {
        ...
        boolean typeMatch = filters.courseType() == null
                || (filters.courseType().equals("free") && (course.basePrice() == null || course.basePrice().signum() == 0))
                || (filters.courseType().equals("paid") && course.basePrice() != null && course.basePrice().signum() > 0);
        boolean levelMatch = filters.level() == null
                || (course.level() != null && course.level().name().equalsIgnoreCase(filters.level()));
        boolean minPriceMatch = filters.minPrice() == null
                || (course.basePrice() != null && course.basePrice().compareTo(filters.minPrice()) >= 0);
        boolean maxPriceMatch = filters.maxPrice() == null
                || (course.basePrice() != null && course.basePrice().compareTo(filters.maxPrice()) <= 0);
