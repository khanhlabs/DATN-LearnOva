...
} from "../../../../../shared/utils/pendingPayOsPayment";
...
import { getCourseDetail, getFileUrl } from "../../../../../shared/api/public/CoursesApi";
import { checkEnrollment } from "../../../../course/infrastructure/api/EnrollmentApi";
import { addWishlistApi, removeWishlistApi, getWishlistApi } from "../../../../course/infrastructure/api/WishlistApi";
...
export default function CourseDetail() {
    ...
    useEffect(() => {
        ...
        const loadCourse = async () => {
            try {
                ...
                const data = await getCourseDetail(courseParam);
                setCourse(data);
    ...
    useEffect(() => {
        if (!courseParam || !isAuthenticated) {
            // eslint-disable-next-line react-hooks/set-state-in-effect
            setEnrolled(false);
            return;
        }

        checkEnrollment(courseParam)
            .then(setEnrolled)
            .catch(() => setEnrolled(false));
    }, [courseParam, isAuthenticated, currentUser]);
    ...
    useEffect(() => {
        ...
        getWishlistApi()
            .then((response) => {
                ...
                setIsWishlisted(items.some((item) => String(item.courseId) === String(courseParam)));
            })
    ...
    const handleToggleWishlist = async () => {
        ...
        const courseId = course.courseId || course.id || courseParam;
    ...
    const handleAddToCart = async () => {
        ...
        try {
            const result = await addCourseToCart(
                {
                    courseId: course.courseId || course.id || courseParam,
                    title: course.title,
    ...
    const handleBuyNow = async () => {
        ...
        try {
            ...
            const courseId = Number(course.courseId || course.id || courseParam);
            ...
            if (
                ...
                navigate(`/learnova/user/courses-detail/${courseParam}`);
                return;
            }
