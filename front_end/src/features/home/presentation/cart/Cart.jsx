...
import { getMyCartApi, removeCartItemApi } from "../../../cart/infrastructure/api/CartApi";
...
const Cart = () => {
  ...
  const handleCheckout = async () => {
    ...
    const voucherCode = (appliedVoucher?.code || promo).trim() || null;

    try {
      setIsCreatingPayment(true);

      const pendingPayment = voucherCode ? null : getPendingPayOsPayment(courseIds);
      if (pendingPayment) {
        setActivePayment({
          ...pendingPayment,
      ...
      toast.error(msg, { autoClose: 4000, toastId: "cart-checkout-error" });
    } finally {
