...
import { claimVoucherApi, getAvailableVouchersApi } from "../../../../../api/VoucherApi.js";
...
const mapVoucher = (voucher) => {
  ...
  return {
    ...
    minimum: formatMoney(voucher.minimumOrder),
    expires: formatDate(voucher.endDate),
    remaining: limit > 0 ? Math.max(limit - used, 0) : "∞",
            ...
            return (
              <article className={`voucher-card voucher-card-${voucher.voucherId} ${isClaimed ? "is-claimed" : ""}`} key={voucher.voucherId}>
                ...
                <div className="voucher-card-content">
                  ...
                  <div className="voucher-meta"><span>{t("profile.vouchers.minimum", { value: voucher.minimum })}</span><span>{t("profile.vouchers.expires", { date: voucher.expires })}</span><span>{t("profile.vouchers.remaining", { count: voucher.remaining })}</span></div>
